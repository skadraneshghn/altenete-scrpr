...
class XenForoAuth:
    ...
    def __init__(self, base_url: str, username: str, password: str, on_session_refreshed=None, logger_cb=None):
    ...
    async def login(self) -> dict:
        ...
        await self.log(f"Attempting login to {self.base_url} as {self.username}")

        try:
            try:
                from playwright.async_api import async_playwright, TimeoutError as PWTimeout
                logger.info("Using Playwright browser-based login for scraper session...")
                await self.log("Using Playwright browser-based login for scraper session...")
                ...
                async with async_playwright() as p:
                    ...
                    await self.log("XenForo login successful via Playwright!")
                    await browser.close()
                ...
                await self.log("Playwright not installed, falling back to httpx-based login...")
                client = get_client()
                ...
                await self.log("CSRF token extracted successfully")
                ...
                if "xf_user" in self._cookies or is_logged_in(login_resp.text):
                    ...
                    await self.log("XenForo login successful!")
            ...
            await self.log(f"XenForo authentication failed: {str(e)}", "error")
            self._is_authenticated = False
            raise
    ...
    async def fetch_with_retry(self, url: str) -> str:
        ...
        await self.log(f"Session expired or guest access returned for {url}. Initiating auto-login...", "warning")
        await self.login()

        # Retry request with new cookies
        logger.info(f"Retrying fetch with updated cookies for: {url}")
        await self.log(f"Retrying fetch with updated cookies for: {url}")
        resp = await client.get(url, cookies=self._cookies)
        resp.raise_for_status()
        html = resp.text

        if not is_logged_in(html):
            msg = "Session expired and auto-login retry did not restore authenticated session."
